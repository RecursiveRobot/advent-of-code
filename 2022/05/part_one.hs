...
applyInstruction (Instruction count from to) stacks
    | count == 0 = stacks
    | otherwise = applyInstruction (Instruction (count - 1) from to) newStacks
      where 
        newStacks = [ stack i | i <- [0..length stacks - 1]]
        stack i | i == from = tail (stacks !! i)
                | i == to = head (stacks !! from) : stacks !! i
                | otherwise = stacks !! i
